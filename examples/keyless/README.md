...
The total-automated-keyless-deployment showcase: shared keys disabled, identity-authenticated
storage, and the FastAPI package pushed by the pipeline's dedicated deploy stage with a fresh
OIDC login. No storage keys, no SAS, anywhere. The environment comes from the Terraform workspace
(`terraform.workspace`), not a variable.
...
| Name | Description |
|------|-------------|
| <a name="output_api_url"></a> [api\_url](#output\_api\_url) | The FastAPI hello world endpoint, live once the deploy stage has pushed the package. |
| <a name="output_function_app_name"></a> [function\_app\_name](#output\_function\_app\_name) | The app name the deploy stage targets. |
| <a name="output_resource_group_name"></a> [resource\_group\_name](#output\_resource\_group\_name) | The resource group the deploy stage targets. |
<!-- END_TF_DOCS -->
