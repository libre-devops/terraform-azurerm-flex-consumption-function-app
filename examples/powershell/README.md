...
The PowerShell worker on the same secure stack: run.ps1 plus function.json, no build step, and an
empty requirements.psd1 so the app needs no gallery access at runtime. The environment comes from
the Terraform workspace (`terraform.workspace`), not a variable.
...
| Name | Description |
|------|-------------|
| <a name="output_api_url"></a> [api\_url](#output\_api\_url) | The PowerShell hello endpoint, live once the deploy stage has pushed the package. |
